...
FIXED VERSION - preserves Course and ISBN columns
"""
...
# Group by Course and ISBN - KEEP THE INDEX COLUMNS
print("Grouping by Course and ISBN...")
grouped = df.groupby(['Course', 'ISBN']).apply(consolidate_group, include_groups=False).reset_index()

# Remove the extra level columns that pandas sometimes adds
...
# Verify columns
print("Columns in consolidated file:")
...
if len(multi_section) > 0:
    ...
    for idx, row in multi_section.head(5).iterrows():
        print(f"  {row['Course']:15} - ISBN: {row['ISBN']}")
        print(f"    Title: {row['Title'][:50]}")
        print(f"    Sections: {row['Section']}")
        print(f"    Instructors: {row['Instructor_Name'][:50]}")
        print()

# Show breakdown by course
print("Unique books per course (after consolidation):")
